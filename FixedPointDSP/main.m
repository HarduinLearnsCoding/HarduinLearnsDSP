...
% Sim paramaters
sim_set=3; % sim set 0 is SNR, sim set 1 is time delay and sim set 2 is freq delay
fs=16000;
f0=0;
t0=0;
...
for it=1:it_num
    ...
    while (FrameError<FERlim && frame<frame_number)
        ...
        data_lp2=conv(data_noisy,hd,'same');  %myConv16
        
        % Sampling delay finder
        for k=0:US-1
            E1(k+1)=sum(mySqr(downsample(data_lp2(k+1:end-k),US))); %mySqr16, %sumall_16 Ismail
        end
        ...
        for i=1:81 %CHANGED 10/29
            E2(i)=max(mySqr(W128*(data_ds(i:i+NumberofPilot-1)))); %MatrixMult Ismail
        end
...
if intpol_par>0
        ...
        [m1a,kinda]=sort(fft_plt,'descend');
        Ks=kinda(1:2);
        Ms=m1a(1:2);
end
...
%         more accurate solution, we also look the second peak. If the 
%         inpol equals zero, we apply 'two points interpolation'. If not
%         and there
%         exists some point larger than half of the peak value, we also
%         take this indice too, 'threshold aimed N points interpolation'.
        
        if length(Ks)>1 % 
            f_estimate=df*(sum(sqrt(Ms).*Ks)/(sum(sqrt(Ms)))-13); %
        else
             f_estimate=df*(Ks-13);
        end
...
legend('Mean of the Frequency Shift Estimation','Actual Frequency Shift Estimation')
    ...
    semilogy(p_all,BER)
    hold on;
    semilogy(p_all,theoric,'o')
    grid on;
    xlim([min(p_all),max(p_all)])
    xlabel('Phase shift (radian)')
    ...
    hold on;
    plot(p_all,p_all)
legend('Mean of the Phase Shift Estimation','Actual Phase Shift')
    grid on;
       ylabel('Phase shift')
    xlabel('Phase shift (radian)')
